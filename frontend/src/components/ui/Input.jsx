export default function Input({ label, error, className = '', icon, ...props }) {
  return (
    <div className="space-y-1.5">
      {label && (
        <label className="block text-sm font-medium text-espresso-600 dark:text-espresso-300">
          {label}
        </label>
      )}
      <div className="relative">
        {icon && (
          <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-espresso-400 dark:text-dark-400">
            {icon}
          </div>
        )}
        <input
          className={`w-full rounded-xl border bg-cream-50 px-4 py-2.5 text-sm text-espresso-800 placeholder:text-espresso-300 transition-all duration-200
            ...
            dark:bg-dark-800 dark:border-dark-700 dark:text-cream-100 dark:placeholder:text-dark-400 dark:focus:bg-dark-800 dark:focus:ring-sage-500/40 dark:focus:border-sage-500 dark:focus:shadow-dark-950/30
            ${icon ? 'pl-10' : ''}
            ${className}`}
          {...props}
        />
