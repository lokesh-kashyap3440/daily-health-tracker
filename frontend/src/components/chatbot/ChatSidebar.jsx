...
export default function ChatSidebar({ sessions = [], activeId, onSelect, onNew }) {
  return (
    <div className="w-64 border-r border-cream-200 p-4 hidden lg:flex flex-col bg-gradient-to-b from-cream-50 to-white dark:border-dark-700 dark:from-dark-900 dark:to-dark-800">
      <button
        onClick={onNew}
      ...
      <div className="flex items-center gap-2 mb-3 px-1">
        ...
        <span className="text-[10px] font-semibold text-espresso-400 uppercase tracking-widest dark:text-dark-400">History</span>
      ...
      <div className="flex-1 overflow-y-auto space-y-1 -mx-1">
        {sessions.length === 0 && (
          <p className="text-xs text-espresso-400 text-center py-8 px-4 dark:text-dark-400">
        ...
        {sessions.map((s) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-left transition-all duration-200 cursor-pointer ${
              s.id === activeId
                ? 'bg-sage-100 text-sage-700 font-medium shadow-sm border border-sage-200 dark:bg-dark-800 dark:text-sage-300 dark:border-dark-700'
          ...
          >
            <MessageCircle size={15} strokeWidth={1.8} className={s.id === activeId ? 'text-sage-500 dark:text-sage-400' : 'text-espresso-400 dark:text-dark-400'} />
